...
### **Amp/Phase 时序图 (Amplitude/Phase vs Time)**

该功能展示了每个虚拟天线的 **幅度 (Amplitude)** 和 **相位 (Phase)** 随时间的变化。它提供了直观的时域信号分析，帮助用户观察信号的稳定性，特别是在进行 **幅相校准** 时，此功能对于验证校准效果尤为重要。
...
- **幅度图 (Amplitude)：**
   绘制信号幅度随时间的变化，显示信号的 **强度**（或者能量）。可以观察到是否存在 **幅度波动**，例如 AGC（自动增益控制）不稳定，或者信号出现 **饱和** 或 **衰减**。
- **相位图 (Phase)：**
...
- **实时更新：**
   每次接收新的数据时，幅度和相位图都会自动更新，确保显示最新的信号状态。每个 **chirp** 会有独立的时序图更新，用户可以看到信号随时间的动态变化。
- **自动缩放：**
